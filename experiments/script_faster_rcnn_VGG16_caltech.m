...
exp_name = 'faster_rcnn_caltech';
% do validation, or not 
opts.do_val                 = true; 
% model
model                       = Model.ZF_for_Faster_RCNN_caltech;
% cache base
cache_base_proposal         = 'faster_rcnn_Caltech_ZF';
cache_base_fast_rcnn        = '';
% train/test data
dataset                     = [];
use_flipped                 = false;
dataset                     = Dataset.caltech_trainval(dataset, 'train30', use_flipped);
dataset                     = Dataset.caltech_test(dataset, 'test30', use_flipped);

%% -------------------- TRAIN --------------------
% conf
conf_proposal               = proposal_config_caltech('image_means', model.mean_image, 'feat_stride', model.feat_stride);
conf_fast_rcnn              = fast_rcnn_config_caltech('image_means', model.mean_image);
