function [miss,roc,gt,dt] = do_test_bf( varargin )
...
dfs={ 'name','REQ', 'roidb_test','REQ','imdb_test','REQ', 'gtDir','REQ', 'pLoad',[], ...
  ...
  'conf' 'REQ', 'caffe_net' 'REQ', 'silent', false, 'cache_dir', 'REQ', 'ratio', 1,'method_name','RPN+BF'};
[name,roidb_test,imdb_test,gtDir,pLoad,thr,mul,reapply,ref,lims,show, nms_thres, conf, caffe_net, silent, cache_dir, ratio, method_name] = ...
...
if(reapply || ~exist(bbsNm,'file'))
  ...
  if ~silent
      cache_dir = fullfile(cache_dir, method_name);
