...
switch usage
    case {'train'}
        dataset.imdb_train    = {  imdb_from_kaist_lwir('./datasets/kaist_lwir', 'train', use_flipped) };
        dataset.roidb_train   = cellfun(@(x) x.roidb_func(x, false), dataset.imdb_train, 'UniformOutput', false);
    case {'test'}
        dataset.imdb_test     = imdb_from_kaist_lwir('./datasets/kaist_lwir', 'train', use_flipped) ;
