function script_rpn_bf_pedestrian_VGG16_kaist_lwir()
% script_rpn_bf_pedestrian_VGG16_kaist_lwir()
% --------------------------------------------------------
% RPN_BF
...
roidb_test_BF = Faster_RCNN_Train.do_generate_bf_proposal_kaist_lwir(conf_proposal, model.stage1_rpn, dataset.imdb_test, dataset.roidb_test);
model.stage1_rpn.nms.nms_overlap_thres = 0.7;
model.stage1_rpn.nms.after_nms_topN = 1000;
roidb_train_BF = Faster_RCNN_Train.do_generate_bf_proposal_kaist_lwir(conf_proposal, model.stage1_rpn, dataset.imdb_train{1}, dataset.roidb_train{1});

%% train the BF
BF_cachedir = fullfile(pwd, 'output', exp_name, 'bf_cachedir');
mkdir_if_missing(BF_cachedir);
dataDir='datasets/kaist_lwir/';
...
pLoad={'lbls',{'person'},'ilbls',{'people','person?','cyclist'}, 'vType', {'none','partial'},'squarify',{3,.41}};
opts.pLoad = [pLoad 'hRng',[55 inf]];
...
if ~exist(folder1, 'dir')
    [~,~,gt,dt]=DeepTest_otf_trans_ratio('name',opts.name,'roidb_test', opts.roidb_test, 'imdb_test', opts.imdb_test, ...
        'gtDir',[dataDir 'test/annotations'],'pLoad',[pLoad, 'hRng',[55 inf],...
        'xRng',[5 635],'yRng',[5 475]],...
        'reapply',1,'show',2, 'nms_thres', opts.nms_thres, ...
        'conf', opts.conf, 'caffe_net', opts.caffe_net, 'silent', false, 'cache_dir', folder1, 'ratio', opts.ratio);
end
