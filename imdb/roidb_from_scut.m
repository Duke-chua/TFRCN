function roidb = roidb_from_scut(imdb, flip)
% roidb = roidb_from_scut(imdb, flip)
...
anno_path = ['./datasets/scut/' roidb.name '/annotations'];
% prop_path = ['./datasets/scut/' roidb.name '/proposals'];

addpath(genpath('./external/code3.2.1'));
pLoad={'lbls',{'walk_person'},'ilbls',{'people','ride_person','person?',...
...
if flip
    ...
    cache_file = ['./imdb/cache/roidb_scut_' imdb.name];
end
