function imdb = imdb_from_scut(root_dir, image_set, flip)
% imdb = imdb_from_scut(root_dir, image_set, flip)
...
if flip
    cache_file = ['./imdb/cache/imdb_scut_' image_set '_flip'];
else
    cache_file = ['./imdb/cache/imdb_scut_' image_set];
end
try
  load(cache_file);
catch
  ...
  for i = 1:length(imdb.image_ids)
    imdb.sizes(i, :) = [576 720]; % the size is fix for scut
  end
  
  imdb.roidb_func = @roidb_from_scut;
  imdb.num_classes = 1;
