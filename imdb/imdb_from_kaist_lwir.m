function imdb = imdb_from_kaist_lwir(root_dir, image_set, flip)
% imdb = imdb_from_kaist_lwir(root_dir, image_set, flip)
...
if flip
    cache_file = ['./imdb/cache/imdb_kaist_lwir_' image_set '_flip'];
else
    cache_file = ['./imdb/cache/imdb_kaist_lwir_' image_set];
end
try
  load(cache_file);
catch
  ...
  imdb.roidb_func = @roidb_from_kaist_lwir;
  imdb.num_classes = 1;
