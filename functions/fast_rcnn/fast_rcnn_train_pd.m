...
%% making tran/val data
    ...
    if opts.do_val
        ...
        % fix validation data
        shuffled_inds_val = generate_random_minibatch([], image_roidb_val, conf.ims_per_batch, conf.fg_image_ratio);
...
%% training
    ...
    while (iter_ < max_iter)
        ...
        [shuffled_inds, sub_db_inds] = generate_random_minibatch(shuffled_inds, image_roidb_train, conf.ims_per_batch,conf.fg_image_ratio);
...
function [shuffled_inds, sub_inds] = generate_random_minibatch(shuffled_inds, image_roidb, ims_per_batch, fg_image_ratio)
    ...
    if isempty(shuffled_inds)
        ...
        if ims_per_batch == 1
            ...
            empty_image_inds = arrayfun(@(x) sum(x.bbox_targets(:, 1)==1) == 0, image_roidb, 'UniformOutput', true);
