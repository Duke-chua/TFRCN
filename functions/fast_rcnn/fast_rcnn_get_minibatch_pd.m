...
function [labels, overlaps, rois, bbox_targets, bbox_loss_weights] = ...
    ...
    gt_inds_full = find(overlaps == 1);
    gt_inds = gt_inds_full(image_roidb.gt_ignores~=1, :);

    rois = image_roidb(1).boxes;
    
    % Select foreground ROIs as those with >= FG_THRESH overlap
    fg_inds = find(overlaps >= conf.fg_thresh & overlaps < 1);
    fg_inds = [gt_inds; fg_inds]; % add gt as foreground ROI
